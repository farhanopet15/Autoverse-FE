...
function MagicCard({
  ...
  gradientOpacity = 0.8,
  gradientFrom = "#D4AF37", // gold
  gradientTo = "#C99700",   // rich gold
}: {
...
export default function RegisterPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setInfo("Akun berhasil dibuat. Silakan login menggunakan email & password.");
    setTimeout(() => { router.replace("/login"); router.refresh(); }, 600);
  };
  ...
  return (
    <main className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-black via-neutral-950 to-black text-neutral-100">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        {/* soft gold vignette */}
        <div className="absolute -top-40 left-1/3 h-96 w-96 rounded-full blur-3xl opacity-20 bg-amber-400/70" />
        <div className="absolute -bottom-48 right-1/4 h-[28rem] w-[28rem] rounded-full blur-3xl opacity-10 bg-yellow-500/60" />
        ...
        {/* subtle carbon texture overlay (optional: provide /luxury-carbon.png) */}
      ...
      <MagicCard className="w-full max-w-md mx-auto">
        <form onSubmit={onSubmit} className="w-full" autoComplete="on">
          <div className="mb-6 text-center">
            <h2 className="text-2xl font-semibold tracking-tight text-amber-300/90">Daftar Akun</h2>
            <p className="text-sm text-neutral-300 mt-1">
              Sudah punya akun? <a href="/login" className="underline underline-offset-4 text-amber-200 hover:text-amber-300">Masuk</a>
            </p>
          </div>
          ...
          {info  && <p className="mt-3 text-sm text-emerald-300/90 bg-emerald-900/20 border border-emerald-800/50 rounded-xl px-3 py-2">{info}</p>}
      ...
      <footer className="absolute bottom-4 w-full text-center text-xs text-neutral-400">
        © {new Date().getFullYear()} Autoverse — Semua hak dilindungi.
      </footer>
    </main>
  );
}